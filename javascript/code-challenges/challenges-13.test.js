...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  return  arr.map(value => value.split("").filter(value => /[0-9]/.test(value)).join(""));
};
...
const onlyOddChars = (str) => {
  // Solution code here...
  return  str.split("").filter((value, idx) => idx % 2 !== 0).join("");
};
...
const allHappy = (arr) => {
  // Solution code here...
  return arr.reduce((acc, value) => /:\)/.test(value) ? acc : acc = false, true);
};
